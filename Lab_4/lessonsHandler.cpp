...
bool lessonsHandler::readNewCmnLessonsArrFromFile(string filePath) {
    ...
    for (int it = 0; it < CSV_rows.size(); it++) {
        lessonStruct currRecord;
        lesson currLesson;
        if (!getCmnRecord(CSV_rows[it], &currRecord)) {
            return 0;
        }
        currLesson.setLessonParams(currRecord);
        cmnLessonArr_ref.push_back(currLesson);
        saveLessonInFile(currRecord, updatedFileName);
    }

//    for (int it = 0; it < lessonArr_ptr->size(); it++) {
//        saveRecordInFile((*lessonArr_ptr)[it].getLessonParams(), filePath);
//        if((lessonsRecords[it].id <= 100) and (lessonsRecords[it].id >= 1)){
//            continue;
//        }
//        lessonsRecords[it].id = getUniqRandID();
//        currID_sortedArr.push_back(lessonsRecords[it].id);
//    }
    return 1;
}
